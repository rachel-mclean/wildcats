...
async function main() {
  let url = 'https://davidsonwildcats.com/sports/mens-soccer/roster';
  ...
  let playerData = Array.from($('.sidearm-roster-players .sidearm-roster-player').map((i, playerListItem) => {
    return {
      ...
      soccer_club: $(playerListItem).find('.sidearm-roster-player-custom1').text().trim(),
      ...
      highschool: $(playerListItem).find('.hide-on-medium-down .sidearm-roster-player-previous-school').text().trim(),
  ...
  //    await knex('mens_soccer_players').insert(player);
  //  }
